...
const getDevServerConfig = (proxy) => {
  ...
  const harData = harFile
    ? har.loadFile(harFile, devPorts ? 8005 : 80, "")
    : undefined;

  return {
    client: {
      webSocketURL: { hostname: "0.0.0.0", port: devPorts ? 8005 : 80 },
    },
    server: {
      type: "https",
      options: {
    ...
    setupMiddlewares(middlewares, devServer) {
      ...
      devServer.webSocketProxies.push({
        upgrade(req, socket, head) {
          const path = Object.keys(socketProxies).find((path) =>
...
const getVirtualModules = (dir, includePkg) => {
  ...
  if (requiredPackages && requiredPackages.dependencies) {
    Object.keys(requiredPackages.dependencies).forEach((requiredPackage) => {
      const library = require(path.join(
        modulePaths,
        requiredPackage,
        "package.json"
      ));
  ...
  return new VirtualModulesPlugin({
    "node_modules/@rancher/dynamic.js": `export default function ($plugin) { ${reqs} };`,
  });
};
...
const getVirtualModulesAutoImport = (dir) => {
  ...
  if (fs.existsSync(pkgFolder)) {
    fs.readdirSync(pkgFolder)
      .filter((name) => !name.startsWith(".")) // Ignore hidden folders
      .forEach((name) => {
        autoImportTypes[`node_modules/@rancher/auto-import/${name}`] =
 ...
 */
const createEnvVariablesPlugin = (routerBasePath, rancherEnv) =>
  new webpack.DefinePlugin({
    ...
    "process.env.loginLocaleSelector": JSON.stringify(
      process.env.LOGIN_LOCALE_SELECTOR || "true"
    ),
    "process.env.excludeOperatorPkg": JSON.stringify(
      process.env.EXCLUDE_OPERATOR_PKG || "false"
    ),
    "process.env.rancherEnv": JSON.stringify(rancherEnv),
    "process.env.harvesterPkgUrl": JSON.stringify(
      process.env.HARVESTER_PKG_URL
    ),
    "process.env.api": JSON.stringify(api),
    "process.env.UI_EXTENSIONS_API_VERSION": JSON.stringify(
      shellPkgData.version
    ),
    ...
    if ("test.js".match(rule.test)) {
      if (rule.exclude) {
        ...
        rule.exclude = function (modulePath) {
          if (
            modulePath.indexOf(SHELL_ABS) === 0 ||
...
const getWatcherIgnored = (excludes = []) => {
  const paths = [/node_modules/, /dist-pkg/, /scripts\/standalone/];
  const pathExcludedPkg = excludes.map(
    (excluded) => new RegExp(`/pkg.${excluded}/`)
  );
  const pathsCombined = [...paths, ...pathExcludedPkg];
  const regexCombined = new RegExp(
    pathsCombined.map(({ source }) => source).join("|")
  );
...
module.exports = function (dir, appConfig = {}) {
  ...
  const config = {
    ...
    configureWebpack(config) {
      ...
      config.resolve.alias["vue$"] = dev
        ? path.resolve(process.cwd(), "node_modules", "vue")
        : "vue";
      ...
      config.plugins.push(
        new CopyWebpackPlugin({
          patterns: [{ from: path.join(SHELL_ABS, "static"), to: "." }],
        })
      );
