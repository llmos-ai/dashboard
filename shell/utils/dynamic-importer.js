...
export function importLogin(name) {
  ...
  return defineAsyncComponent(() =>
    import(
      /* webpackChunkName: "login" */ `@shell/components/auth/login/${name}`
    )
  );
}
...
export function importList(name) {
  ...
  return defineAsyncComponent(() =>
    import(/* webpackChunkName: "list" */ `@shell/list/${name}`)
  );
}
...
export function importDetail(name) {
  ...
  return defineAsyncComponent(() =>
    import(/* webpackChunkName: "detail" */ `@shell/detail/${name}`)
  );
}
...
export function importEdit(name) {
  ...
  return defineAsyncComponent(() =>
    import(/* webpackChunkName: "edit" */ `@shell/edit/${name}`)
  );
}
...
export function importDialog(name) {
  ...
  return defineAsyncComponent(() =>
    import(/* webpackChunkName: "dialog" */ `@shell/dialog/${name}`)
  );
}
...
export function importWindowComponent(name) {
  ...
  return defineAsyncComponent(() =>
    import(
      /* webpackChunkName: "components/nav" */ `@shell/components/nav/WindowManager/${name}`
    )
  );
}
...
export function loadProduct(name) {
  ...
  return import(
    /* webpackChunkName: "product" */ `@shell/config/product/${name}`
  );
}
...
export function listProducts() {
  ...
  const products = ctx
    .keys()
    .filter((path) => !path.endsWith(".js") && path.startsWith("./"))
    .map((path) => path.substr(2));
...
export function loadTranslation(name) {
  ...
  // Note: directly returns the import, not a function
  return import(
    /* webpackChunkName: "[request]" */ `@shell/assets/translations/${name}.yaml`
  );
}
...
export function importCustomPromptRemove(name) {
  return defineAsyncComponent(() =>
    import(
      /* webpackChunkName: "custom-prompt-remove" */ `@shell/promptRemove/${name}`
    )
  );
}
...
/**
 * Resolves the module path for a window component by its key.
 *
 * @param {string} key - The key representing the window component.
 * @returns {string} The resolved path to the window component module.
 */
