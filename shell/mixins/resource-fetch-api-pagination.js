  ...
  watch: {
    ...
    namespaceFilters: {
      ...
      async handler(neu, old) {
        ...
        const neuEmpty =
          !neu || neu.length === 0 || neu[0] === NAMESPACE_FILTER_ALL;
        const oldEmpty =
          !old || old.length === 0 || old[0] === NAMESPACE_FILTER_ALL;

        if (neuEmpty && oldEmpty) {
          const allButHidingSystemResources =
    ...
    async pagination(neu, old) {
      ...
      if (
        neu &&
        this.$options.name !== ResourceListComponentName &&
        !!this.$fetch &&
        !this.paginationEqual(neu, old)
      ) {
    ...
    async paginationResult(neu, old) {
      if (!this.fetchPageSecondaryResources || !neu) {
        // || neu.timestamp === old?.timestamp
        return;
      }
