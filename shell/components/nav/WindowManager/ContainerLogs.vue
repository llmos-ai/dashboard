...
export default {
  ...
  computed: {
    containerChoices() {
      ...
      const initContainers = (this.pod?.spec?.initContainers || []).map(
        (x) => x.name
      );
      ...
      <div class="wm-button-bar">
        ...
        <div class="status log-action p-10">
          <t
            ...
            :k="
              isOpen ? 'wm.connection.connected' : 'wm.connection.disconnected'
            "
          />
        </div>
      </div>
    ...
    <template #body>
      <div
        ...
        :class="{
          ...
          open: isOpen,
          closed: !isOpen,
          'show-times': timestamps && filtered.length,
          'wrap-lines': wrap,
        }"
      >
        <VirtualList
