...
<template>
  <a-dropdown trigger="click">
    <a
      class="ant-dropdown-link px-[6px] cursor-pointer"
      @click.prevent
    >
    ...
    <template #overlay>
      <a-menu>
        <a-menu-item
          v-for="option in pageActions"
          :key="option.action"
        >
          <a-divider v-if="option.separator" />
          <span
            v-else-if="option.labelKey"
            @click="pageAction($event, option.action)"
          >
            {{ t(option.labelKey) }}
          </span>
          <span
            v-else
            @click="pageAction($event, option.label)"
          >{{ option.label }}</span>
        </a-menu-item>
      </a-menu>
    </template>
