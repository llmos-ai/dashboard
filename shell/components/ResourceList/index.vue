  ...
  async fetch() {
    ...
    if (this.hasListComponent) {
      ...
      if (component?.$loadingResources) {
        const { loadResources, loadIndeterminate } =
          component?.$loadingResources(this.$route, this.$store);
    ...
    if (!this.componentWillFetch) {
      ...
      if (
        !this.namespaceFilterRequired &&
        (!this.canPaginate || this.refreshFlag)
      ) {
...
<template>
  ...
  >
    ...
    <template v-if="hasListComponent">
      <component
        :is="listComponent"
