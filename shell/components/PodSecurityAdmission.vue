...
export default defineComponent({
  ...
  data() {
    return {
      ...
      psaControls: toDictionary(PSAModes, getPsaControl) as Record<
        PSAMode,
        PSAControl
      >,
      psaExemptionsControls: toDictionary(
        PSADimensions,
        getExemptionControl
    ...
    >
      ...
      <span class="col span-4">
        <LabeledInput
          ...
          :options="options"
          :placeholder="
            t('podSecurityAdmission.version.placeholder', { psaControl: mode })
          "
          :mode="mode"
          :aria-label="`${t(`podSecurityAdmission.labels.${level}`)} - ${t(
            'podSecurityAdmission.version.placeholder',
            { psaControl: mode }
          )}`"
          @update:value="updateLabels()"
        />
      ...
      >
        <span class="col span-2">
          <Checkbox
            ...
            :data-testid="
              componentTestid + '--psaExemptionsControl-' + i + '-active'
            "
            :label="dimension"
        ...
        <span class="col span-8">
          <LabeledInput
            ...
            :data-testid="
              componentTestid + '--psaExemptionsControl-' + i + '-value'
            "
            :disabled="isView || !psaExemptionsControl.active"
            :options="options"
            :placeholder="
              t('podSecurityAdmission.exemptions.placeholder', {
                psaExemptionsControl: dimension,
              })
            "
            :mode="mode"
            :aria-label="`${t(
              `podSecurityAdmission.labels.${dimension}`
            )} - ${t('podSecurityAdmission.exemptions.placeholder', {
              psaExemptionsControl: dimension,
            })}`"
            @update:value="updateExemptions()"
